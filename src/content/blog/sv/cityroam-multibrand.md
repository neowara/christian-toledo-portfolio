...
Det fungerar nu. Elsparkcykeln parkopplas, autentiserar, strömmar telemetri och tar emot kommandon: körläge, energiåtervinning, lås, farthållare och lysen fungerar alla från samma app, på samma turinspelare, dashboard, widget och notiser som brädan redan använde. Det här inlägget handlar om vad som faktiskt låg i vägen, för nästan inget av det var den del jag väntade mig.
...
NAVEE — vars elsparkcyklar faktiskt tillverkas av en OEM vid namn Brightway Innovation Intelligent Technology — har ingen motsvarighet alls. Deras app pratar med elsparkcykeln över ett proprietärt, odokumenterat BLE-protokoll och med deras eget backend över ett REST-API utan publik dokumentation, utan API-nycklar för tredjepart och utan publicerat SDK. Här fanns inget leverantörs-SDK att ta bort, för det fanns aldrig något från början. Det var ett rent reverse-engineering-problem från första raden.
...
Den referensen var genuint värdefull och innehöll också den enskilda detalj som kostade mest tid.
...
Gör man det baklänges misslyckas handskakningen på minsta möjliga informativa sätt — elsparkcykeln slutar helt enkelt svara, länken bryts, och appen återansluter och gör om det, i all evighet.
...
4. Elsparkcykeln svarar på `0x30` med en 16 byte lång utmaning. Appen svarar `0x31` med den utmaningen **AES-128-ECB-krypterad** under nyckeln den angav. Elsparkcykeln svarar `0x31` med status 0. Appen skickar `0x30` igen; elsparkcykeln svarar `0x30`, status 0, ingen utmaning — autentiserad.
...
Det fanns alltså två oberoende orsaker till samma symptom, och en återanslutningsloop ovanpå som ivrigt dolde båda. Lösningen var inte smart; den bestod i att läsa vad den officiella appen faktiskt gör, i ordning, och göra exakt det och inget mer.
...
Vinsten är att bara två saker är genuint märkesspecifika: BLE-protokollmodulen och konto-API:et. Allt ovanför det — sessionslagret, turinspelaren, den nativa turjournalen, dashboarden, hemskärmswidgeten, notiserna — körs oförändrat på båda enheterna. En elsparkcykel spelar in en tur genom exakt samma kodväg som en bräda.
